...
|---|---|---|---|
| `2010` (default) | Tuesday 09:30 | A supplier trailer docked at an inbound door with the receiver beside it, a store trailer docked at an outbound door, the forklift operator in the reserve, queue chips where the work waits (`unload` and `receive` at the first inbound door, `putaway` at the forklift park beside it, `replenish` on the reserve's front corridor), a picker at the pick module | Trucks, doors, lanes, reserve, chips: the most checks in one frame |
...
- Measured on the dev box (Intel UHD via ANGLE D3D11, 1600 × 1000) at integration: dc-west 1.1 ms, 62 draw calls, 42.2 k triangles; dc-east 0.8 ms, 86 draw calls, 67.1 k triangles (73 / 64.1 k before the environment's sky, ground, fence and poles); the CSV sample 0.9 ms, 95 draw calls, 55.3 k triangles. Re-run `perf-twin.mjs` after any scene change and update these. `frameMs` is the main-thread time of one `viewer.frame()` (sampling the playback, applying it to the scene, submitting the draw calls and projecting the labels), which is what the page can measure; the GPU's own time is not in it, so the number says the CPU side has room, not that a 4K canvas with shadows hits 60 fps.
...
- Light theme and no night lighting: the floor is cream, the walls warm grey, the yard mid-grey; no dim blue cast, no glowing ceiling strips, no shadows (shadows are off in shot mode). Around the site: a blue-white sky gradient above, an olive ground plane past the asphalt lot, a perimeter fence and light poles along the lot. A black frame, or black above the building, means the shot never reached `twin:ready` (see "When a check fails"), not an expected void.
...
- Labels: a white pill with dark text over every worker on the floor (`W-E-003`) and every trailer that is docked or moving, named after the supplier or the store (`Domestic hard candy and mint supplier truck`, `Buford Highway World Sweets truck`; the PO id is in the inspector and the ticker's parentheses); blue pills with the queued count where the work waits (`receive 1` at the inbound door, `replenish 6` on the reserve's front corridor). Text is crisp and legible; a label may overlap a neighbour when two actors share a spot, but none is clipped or upside down. From a low camera outside the building (the `dock` preset) no worker pill floats on a blank wall: the walls hide them until the camera rises above the wall top.
...
- The queue spots sit beyond the road (75 ft out, 70 ft apart): a waiting truck never overlaps the docked trailer, the road or the next truck in line.
- Beyond the road: the asphalt lot, then the olive ground plane, the fence and the light poles; a black band there means the environment did not load.
...
- `1000x600` (the narrowest app-mode window, optional): the stage is too narrow for the toolbar and the clock side by side, so the clock pill drops under the toolbar at the right and the HUD strip under that; the toolbar itself never wraps and nothing overlaps. The minimap is 170 px at 640 px tall or less. The floor is cramped at this size; H and M hide the HUD and minimap.
...
- What a capture cannot show, check by hand in a browser: More expands the HUD in place (every tile, the queue bars and today's trucks) and Less or Esc collapses it; the View menu and the timeline's `?` legend open as popovers and close on Esc or a click elsewhere; Got it removes the hint and it stays gone after a reload (localStorage `twin.hint.v1`); clicking a toolbar button never orbits or picks on the canvas underneath; after a mouse click on any button over the stage or on the transport row, Space still plays or pauses (focus never stays on the button; Tab focus works as usual).
