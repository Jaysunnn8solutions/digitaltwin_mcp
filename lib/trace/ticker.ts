 ...
 * frequent to list; trucks, orders, people and the system are.
 */
...
export function tickerLine(e: TraceEvent, names: TickerNames): { text: string; severity: 0 | 1 | 2 } | null {
  switch (e.k) {
    ...
    case "poPlaced":
      return { text: `PO ${e.po} placed with ${e.supplier}: ${e.pallets} pallet${e.pallets === 1 ? "" : "s"}, due day ${e.arriveDay + 1}.`, severity: 0 };
    case "truckScheduled":
      return { text: `${e.supplier} truck for ${e.po} appointed ${clock(e.appointment)}, expected ${clock(e.eta)}.`, severity: 0 };
    case "truckArrive":
      return { text: `${e.importer ? "Import" : "Supplier"} truck ${e.po} arrives with ${e.pallets.length} pallet${e.pallets.length === 1 ? "" : "s"}.`, severity: 1 };
    case "truckDock":
      return { text: `${e.po} docks${e.waitMin > 0.5 ? ` after waiting ${Math.round(e.waitMin)} min for a door` : ""}.`, severity: e.waitMin > 30 ? 2 : 0 };
    case "truckUndock":
      return { text: `${e.po} is unloaded and leaves the door.`, severity: 0 };
    case "short":
      return { text: `Face short: ${names.sku(e.sku)} for ${e.order}, ${e.inners} inner${e.inners === 1 ? "" : "s"} missing${e.hot ? ", hot replenishment requested" : ""}.`, severity: 2 };
    case "shortShip":
      return { text: `${e.order} ships short ${e.inners} inner${e.inners === 1 ? "" : "s"} of ${names.sku(e.sku)}: nothing left anywhere.`, severity: 2 };
    case "orderRelease": {
    ...
    case "orderPicked":
      return { text: `${e.order} fully picked: ${e.inners} inners on ${e.pallets} pallet${e.pallets === 1 ? "" : "s"}.`, severity: 0 };
    case "orderCut":
      return { text: `${e.order} cut entirely: no truck.`, severity: 2 };
    case "truckLoaded":
      return e.lateMin > 0
    ...
    case "truckDepart":
      return { text: `${e.order} departs.`, severity: 0 };
    case "worker":
      switch (e.state) {
        case "in":
          return { text: `${e.id} clocks in on ${e.shift ?? "shift"} as ${e.primary ?? "crew"}.`, severity: 0 };
        case "absent":
