...
**Optimizer.** optimize_operations (and the 3D page's Optimize tab, the same code in the browser's Web Worker) runs a genetic algorithm over the operation's levers. The genome is one gene per lever: full-time and part-time order selectors, receivers, forklift operators and loaders added; cross-training selectors on pack, receivers on forklift, loaders on pick; flexing; the overtime cap; slotting and pick-face cases; forklifts, pallet jacks, inbound and outbound doors; the cycle service level and the forecast; and the store trucks' departure time. Every genome is a scenario the twin already accepts, written on top of the base scenario, so demand, outages, an imported layout or a candystore store scenario apply to every candidate. Fitness is one weekly cost: the engine's labor cost, hires and cross-training amortized (a year and six months), equipment and doors beyond the building's own, the carrying cost of the average stock held (an annual rate on its cost value, charged weekly) and priced penalties for late trucks (flat plus per minute), trucks never loaded and orders cut for lack of stock. Three presets differ only in how dearly a late truck is priced (service first, balanced, cost first); every price is an input and is reported with the plan. Generation 0 is the operation as it is, every one-lever step from it and random plans; each generation keeps the elite, breeds children of tournament winners with uniform crossover and per-gene mutation, adds one random immigrant, and stops early once the best has not improved for eight generations. Every candidate is evaluated with the engine seeds 1..n (the same for every candidate) and memoized by its genes, so the converged tail is cheap and the winner replays exactly on the 3D page with seed 1. The search is deterministic for a spec and seed. The result is the best plan found by those prices, not a proof of optimality.
